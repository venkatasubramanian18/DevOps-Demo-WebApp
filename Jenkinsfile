...
//	}
		...
		steps {
			withSonarQubeEnv(credentialsId: SonarCredential, installationName: SonarInstallationName) { 
				sh 'mvn clean package sonar:sonar -Dsonar.host.url=${env.JENKINS_URL}:9000 -Dsonar.sources=. -Dsonar.tests=. -Dsonar.inclusions=**/test/java/servlet/createpage_junit.java -Dsonar.test.exclusions=**/test/java/servlet/createpage_junit.java -Dsonar.login=$SONAR_CRED_USR -Dsonar.password=$SONAR_CRED_PSW'
			}
